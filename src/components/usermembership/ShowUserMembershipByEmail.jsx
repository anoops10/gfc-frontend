...
import {
  deleteUserMembershipByumID,
  showUserMembershipByID,
...
export const ShowUserMembershipByEmail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const trainerResponse = await findTrainerByTId(
        clientResponse?.trainer?.tId
      );
  ...
  return (
    <Container className="my-5">
      <h2 className="text-center mb-4 fw-bold" style={{ color: "#526302" }}>
        Membership Details
      </h2>
      {userData?.userMembershipList?.length === 0 ? (
        <h3 className="text-center text-muted">
          No Membership available for this user
        </h3>
      ) : (
                      ...
                      <div className="d-flex flex-column gap-2">
                        <div className="d-flex gap-2 justify-content-center">
                          <Button
                            ...
                            size="sm"
                            onClick={async() => {
                             await deleteUserMembershipByumID(data?.umId);
                            }}
                          >
                            Delete
                          </Button>
                          <Button
                            size="sm"
                            style={{
                              backgroundColor: "#526302",
                              borderColor: "#526302",
                              color: "white",
                            }}
                            onClick={() => {
                              setTrainer(true);
                              setSelectedUmId(data?.umId);
                            }}
                          >
                            Assign Trainer
                          </Button>
            ...
            <Table striped bordered hover className="mb-0">
              ...
              <tbody>
                <tr className="align-middle">
                  <td>
                    <img
                      src={getImage(trainerData?.user?.uId)}
                      alt={trainerData?.user?.uName}
                      width="100"
                      height="100"
