  ...
  return (
    <div style={{ backgroundColor: '#526302', padding: '20px', borderRadius: '8px' }}>
      ...
      <Card className="shadow-sm">
        ...
        <Table striped bordered hover responsive className="mb-0">
          <tbody>
            ...
            <tr>
              <td className="fw-bold">Specialisation</td>
              <td>{trainerData?.mem?.mName}</td>
            </tr>
            <tr>
              <td className="fw-bold">Experience</td>
              <td>{trainerData?.expYears} Years</td>
            </tr>
            <tr>
              <td className="fw-bold">Bio</td>
              <td>{trainerData?.bio}</td>
            </tr>
            {role === "Admin" && <tr>
              <td className="fw-bold">Clients</td>
              <td>
                <div className="d-flex flex-wrap gap-2">
                  {(trainerData?.clientList?.map((client) => (
                    <Button key={client.clientId} variant="outline-primary" size="sm" onClick={() => { setClientId(client.clientId) }}>{client?.user?.uName}</Button>
                  )))}
                </div>
              </td>
            </tr>}
      ...
      {clientId && details && (
        ...
        <Button variant="warning" className="mt-3" onClick={()=>{setUpdate(true); setDetails(false);}}>Update Client</Button>
        </div>
      )}
      {clientId && update && (
        <div className="mt-4 p-3 bg-white rounded shadow-sm">
          <UpdateClient id={clientId} onUpdateSuccess={() => {setUpdate(false); setDetails(true);}}/>
          <Button variant="info" className="mt-3" onClick={()=>{setUpdate(false); setDetails(true);}}>Back to Details</Button>
      </div>
      )}
    </div>
  )
